/**
 * Antigravity History Fix — Extension entry point.
 *
...
export async function activate(context: vscode.ExtensionContext) {
    output = vscode.window.createOutputChannel('Antigravity History Fix');
        ...
        for (const r of fixResults) {
            ...
            if (r.status === 'version-blocked') {
                vscode.window.showWarningMessage(
                    `Antigravity History Fix: auto-run fix skipped — unsupported AG version. ${r.error}`,
                );
            }
        }
    });
    ...
    applyLinkApproval().then(fixResults => {
        for (const r of fixResults) {
            ...
            if (r.status === 'version-blocked') {
                vscode.window.showWarningMessage(
                    `Antigravity History Fix: link-approval fix skipped — unsupported AG version. ${r.error}`,
                );
            }
        }
    });
